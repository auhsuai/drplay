...
describe("HomeTab Recently Added mirror query (db.files source)", () => {
  ...
  it("M4. android: missing modifiedTime sorts oldest and .m4a stays excluded (ExoPlayer non-faststart)", async () => {
    // Variant of the same root cause: the mirror sort must treat a missing
    // modifiedTime as oldest, and the Android playable set (no .m4a) must be
    // the filter — not the PC list that still contains .m4a.
    mocks.dbFilesToArray.mockResolvedValue([
      mirrorRow({ id: "no-ts", name: "NoTs.mp3", modifiedTime: undefined }),
      mirrorRow({
        id: "new",
        name: "New.mp3",
        modifiedTime: "2026-03-01T00:00:00Z",
      }),
      mirrorRow({ id: "m4a", name: "Clip.m4a" }),
      mirrorRow({ id: "aac", name: "Tone.aac" }),
    ]);
    render(<HomeTab {...baseProps()} />);

    await screen.findByText("New.mp3");
    const cards = await screen.findAllByTestId("premium-card");
    // Newest first, missing modifiedTime last; .m4a filtered on Android.
    expect(cards.map((c) => c.textContent)).toEqual([
      "New.mp3",
      "Tone.aac",
      "NoTs.mp3",
    ]);
    expect(screen.queryByText("Clip.m4a")).toBeNull();
  });
});
...
describe("HomeTab Recently Added View All (overlay reuses Recent Files mechanism)", () => {
  ...
  it('d. exactly 5 items (= visibleCount desktop) shows the overlay and opens full view with all 5 (contract flip: was "no overlay")', async () => {
    // Regression (a): 5 items == visibleCount means the mirror page was full
    // (limit=100 capped), so more files may exist behind it. The last card
...
describe("HomeTab mobile: horizontal snap scroll for non-ViewAll sections (Task 7)", () => {
  ...
  it("2. mobile: Recent Files + Recently Added (View All overlay) keep the fixed grid", async () => {
    ...
    mocks.dbFilesToArray.mockResolvedValue([
      mirrorRow({ id: "a1", name: "Added One.mp3" }),
    ]);
    render(<HomeTab {...baseProps()} />);
