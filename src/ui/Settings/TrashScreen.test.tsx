...
vi.mock('../../utils/drivePagination', () => ({ getTrashedFiles: mocks.getTrashedFiles }));
...
type DeferredCall = {
  resolve: (value: Array<{ id: string; name: string; mimeType: string }>) => void;
...
function installGetTrashedFilesMock() {
  mocks.getTrashedFiles.mockImplementation(
    () =>
      new Promise<Array<{ id: string; name: string; mimeType: string }>>((resolve, reject) => {
...
describe('TrashScreen skeleton loading', () => {
  ...
  it('renders the real item list after loading finishes', async () => {
    ...
    await act(async () => {
      deferredCalls[0].resolve([{ id: 'f1', name: 'Track 1', mimeType: 'audio/mpeg' }]);
    });
  ...
  it('never flashes the "Trash is empty" state before the skeleton (first commit is already loading)', async () => {
    ...
    const recordMarkers = () => {
      ...
      const hasEmpty = (document.body.textContent ?? '').includes('settings.trash_empty');
