...
export function LanguageDropdown() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        <span className="truncate group-hover:text-[#4285F4] transition-colors">{currentLang.label}</span>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-44 bg-white dark:bg-[#2a2b2f] rounded-xl shadow-xl p-1 z-50 flex flex-col transform origin-top-right transition-all animate-in fade-in zoom-in-95 duration-200">
          {languages.map(lang => (
            ...
            >
              <span className={`truncate ${currentLang.code === lang.code ? 'text-[#4285F4] font-semibold' : 'text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white transition-colors'}`}>
                {lang.label}
              </span>
