...
import { Skeleton, SkeletonText, SkeletonCardGrid, SkeletonRowList } from './Skeleton';
...
describe('SkeletonCardGrid', () => {
  ...
  it('container uses the exact HomeTab grid classes', () => {
    ...
    expect(grid.className).toContain('grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6');
  });
...
describe('SkeletonRowList', () => {
  ...
  it('renders the requested number of rows, each with an icon and two text lines', () => {
    ...
    const row = container.querySelector('[data-testid="skeleton-row"]') as HTMLElement;
...
describe('SkeletonRowList variants (dimensions mirror the real list rows)', () => {
  ...
  it('stretch still adds flex-1 to rows of every variant', () => {
    const { container } = render(<SkeletonRowList rows={2} variant="trash" stretch />);
    const list = container.firstChild as HTMLElement;
    expect(list.className).toContain('h-full');
    for (const row of Array.from(container.querySelectorAll('[data-testid="skeleton-row"]'))) {
  ...
  it('every variant icon skeleton carries the ring border mirroring the real icon box bg boundary', () => {
    ...
    for (const variant of variants) {
      const { container } = render(<SkeletonRowList rows={1} variant={variant} />);
      const row = container.querySelector('[data-testid="skeleton-row"]') as HTMLElement;
      const icon = variant === 'trash'
...
describe('SkeletonRowList stretch (fills the loading region)', () => {
  ...
  it('stretch adds h-full to the container and flex-1 to every row', () => {
    ...
    const list = container.firstChild as HTMLElement;
    expect(list.className).toContain('h-full');
    const rows = Array.from(container.querySelectorAll('[data-testid="skeleton-row"]'));
  ...
  it('merges the caller className with the stretch classes', () => {
    const { container } = render(<SkeletonRowList rows={2} stretch className="flex-1" />);
    const list = container.firstChild as HTMLElement;
    expect(list.className).toContain('h-full');
...
describe('Skeleton accessibility', () => {
  ...
  it('marks both new component containers as aria-hidden', () => {
    ...
    expect((gridContainer.firstChild as HTMLElement).getAttribute('aria-hidden')).toBe('true');
    const { container: listContainer } = render(<SkeletonRowList />);
    expect((listContainer.firstChild as HTMLElement).getAttribute('aria-hidden')).toBe('true');
  });
});
