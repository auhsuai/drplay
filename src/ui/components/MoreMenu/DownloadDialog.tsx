...
export function DownloadDialog({ isOpen, fileName, isDownloading, onFileNameChange, onDownload, onClose }: DownloadDialogProps) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={e => e.stopPropagation()}>
      <div className="bg-white dark:bg-[#1a1b1e] rounded-2xl p-6 w-full max-w-md shadow-2xl flex flex-col gap-5 animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => !isDownloading && onClose()} disabled={isDownloading}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('menu.file_name')}</label>
        ...
        <div className="flex items-center justify-end gap-3 mt-2">
          ...
          <button onClick={onDownload} disabled={isDownloading || !fileName.trim()}
            ...
            <span>{isDownloading ? t('menu.downloading') : t('menu.confirm_download')}</span>
          </button>
        </div>
      </div>
    </div>,
