...
export function BulkDeleteConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={() => !isOperating && onClose()}>
      <div className="bg-white dark:bg-[#1a1b1e] rounded-2xl p-6 w-full max-w-md shadow-2xl flex flex-col gap-5 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('drive.bulk_delete_title', 'Xác nhận xóa hàng loạt')}</h3>
          <button 
            onClick={onClose}
        ...
        <div className="text-gray-500 dark:text-gray-400 text-sm">
          {t('drive.bulk_delete_desc', 'Bạn có chắc chắn muốn xóa {{count}} mục đã chọn không? Các mục này sẽ được chuyển vào Thùng rác Google Drive.', { count: selectedCount })}
        </div>
