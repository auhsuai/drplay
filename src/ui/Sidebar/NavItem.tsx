...
export function NavItem({
  ...
  return (
    ...
    >
      <div
        className={`w-6 h-6 flex items-center justify-center shrink-0 transition-colors ${active ? "text-brand-primary" : "opacity-70 group-hover:text-brand-primary group-hover:opacity-100"}`}
      >
        {icon}
      </div>
      <div
      ...
      >
        <span className="text-sm block truncate group-hover:text-brand-primary">
          {label}
        </span>
      </div>
    </div>
