...
vi.mock('lucide-react', () => {
  const icons = ['Home', 'HardDrive', 'Settings', 'Heart', 'Plus', 'ListMusic', 'LogOut', 'Gauge'];
  const Stub = () => null;
...
function quotaTextContent(): string | null {
  return screen.getByTestId('storage-quota-text').querySelector('p')?.textContent ?? null;
}
...
describe('Sidebar storage quota', () => {
  ...
  it('fetches quota on mount and renders a bar + "X GB / Y GB" text', async () => {
    ...
    expect(screen.getByTestId('storage-quota-track').className).toContain('bg-gray-200');
    expect(screen.getByTestId('storage-quota-track').className).toContain('dark:bg-[#2A2A2A]');
    expect(screen.getByTestId('storage-quota-track').className).not.toContain('dark:bg-gray-700');
    // Flex layout so the blue + red segment divs sit side by side (no gap).
    expect(screen.getByTestId('storage-quota-track').className).toContain('flex');
    ...
    expect(screen.getByTestId('storage-quota-track').className).toContain('ml-3');
  ...
  it('renders nothing when the sidebar is collapsed and limit is absent (bar needs a limit)', async () => {
    ...
    await waitFor(() => expect(mocks.getDriveStorageQuota).toHaveBeenCalledTimes(1));
    expect(screen.queryByTestId('storage-quota')).toBeNull();
    expect(screen.queryByTestId('storage-quota-bar')).toBeNull();
  });
  ...
  it('clamps the two segments to 100% total when usage exceeds the limit (no layout break)', async () => {
    mocks.getDriveStorageQuota.mockResolvedValue(makeQuota({ limit: 100 * GB, usageInDrive: 150 * GB }));
    render(<Sidebar {...baseProps()} />);
  ...
  it('fills a single blue bar (usage width) with blue text when usage is at or under the 80% threshold', async () => {
    mocks.getDriveStorageQuota.mockResolvedValue(makeQuota({ limit: 100 * GB, usageInDrive: 60 * GB }));
    render(<Sidebar {...baseProps()} />);
  ...
  it('splits the fill into a blue safe zone (80%) + red excess (10%) with red text when usage crosses the 80% threshold', async () => {
    mocks.getDriveStorageQuota.mockResolvedValue(makeQuota({ limit: 100 * GB, usageInDrive: 90 * GB }));
    render(<Sidebar {...baseProps()} />);
  ...
  it('treats exactly 80% usage as safe (blue fill, blue text)', async () => {
    mocks.getDriveStorageQuota.mockResolvedValue(makeQuota({ limit: 100 * GB, usageInDrive: 80 * GB }));
    render(<Sidebar {...baseProps()} />);
  ...
  it('shows the blue safe-zone + red excess segments in the collapsed track too when over threshold', async () => {
    ...
    expect(screen.getByTestId('storage-quota-track').className).toContain('w-11');
    expect(quotaTextClass()).toContain('text-red-500');
  });
  ...
  it('shows only a compact bar (no number) when the sidebar is collapsed', async () => {
    ...
    expect(screen.getByTestId('storage-quota').getAttribute('title')).toContain('2 GB / 15 GB');
  });
  ...
  it('re-fetches quota on the user-changed event', async () => {
    ...
    await waitFor(() => expect(mocks.getDriveStorageQuota).toHaveBeenCalledTimes(2));
  });
  ...
  it('hides the section when the token prop changes to null (logout)', async () => {
    ...
    await waitFor(() => expect(screen.queryByTestId('storage-quota')).toBeNull());
  });
  ...
  it('fires the create-playlist flow unchanged (no regression on existing behavior)', async () => {
    mocks.createPlaylist.mockResolvedValue({ id: 'pl-new', name: 'New', userEmail: 'u', createdAt: 0, tracks: [] });
    mocks.getDriveStorageQuota.mockResolvedValue(makeQuota());
    const onTabChange = vi.fn();
    render(<Sidebar {...baseProps({ onTabChange })} />);

    fireEvent.click(screen.getByTitle('sidebar.create_playlist'));
    const input = screen.getByPlaceholderText('sidebar.new_playlist_placeholder');
    fireEvent.change(input, { target: { value: 'New' } });
    fireEvent.submit(input);

    await waitFor(() => expect(onTabChange).toHaveBeenCalledWith('playlist_pl-new'));
  });
});
