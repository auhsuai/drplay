...
// Cold-start resilience for the best-effort profile fetch: on slow Android
// networks the single attempt used to fail and leave USER_EMAIL_KEY unset for
// the whole session, which permanently blocked the pro-sync sentinel guard
          ...
          try {
            ...
            if (!isLastAttempt) {
              void captureError({
                level: "warn",
                source: AUTH_MODULE,
                message: `Profile fetch attempt ${String(attempt)}/${String(PROFILE_FETCH_RETRY_MAX)} failed (best-effort): ${message}`,
              });
            } else {
              void captureError({
                level: "error",
