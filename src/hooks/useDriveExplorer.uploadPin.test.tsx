...
function renderExplorer() {
  return renderHook(() => useDriveExplorer(FOLDER_ID, 'Folder', TOKEN, () => {}));
}
...
describe('useDriveExplorer upload pin: uploading items are pinned to the top', () => {
  it('pins an uploading item to the top of the list despite the name sort (default name_natural)', async () => {
    await db.files.bulkPut([makeFile('id-a', 'A.mp3'), makeFile('id-z', 'Zik On Air.mp3')]);
    ...
    expect(result.current.filteredItems.map(i => i.id)).toEqual(['id-z', 'id-a']);
    expect(result.current.currentItems.map(i => i.id)).toEqual(['id-z', 'id-a']);
  });

  it('does NOT pin a folder whose child is uploading (parent-uploading sorts normally)', async () => {
    await db.files.bulkPut([makeFile('id-afolder', 'AFolder', true), makeFile('id-zfolder', 'ZFolder', true)]);
    ...
    expect(result.current.filteredItems.map(i => i.id)).toEqual(['id-afolder', 'id-zfolder']);
  });
  ...
  it('does NOT pin when a global search query is active (search results keep their own sort)', async () => {
    await db.files.bulkPut([makeFile('id-a', 'A.mp3'), makeFile('id-z', 'Zed.mp3')]);
    mockUploadStates({ 'id-z': 'uploading' });
    ...
    expect(result.current.filteredItems.map(i => i.id)).toEqual(['id-a', 'id-z']);
  });
});
...
describe('useDriveExplorer upload pin: version bump re-sorts on upload state change', () => {
  it('pins the item as soon as an upload starts (subscribe fires -> re-sort)', async () => {
    await db.files.bulkPut([makeFile('id-a', 'A.mp3'), makeFile('id-z', 'Z.mp3')]);
    mockUploadStates({});
