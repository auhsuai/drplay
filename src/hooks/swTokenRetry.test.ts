...
interface FakeSelf {
  ...
  makeFetchEvent: (fileId: string) => { request: Request; respondWith: ReturnType<typeof vi.fn> };
}
...
describe('sw.js /drive-stream/ 401 recovery', () => {
  it('notifies clients and retries exactly once with the fresh token after UPDATE_TOKEN', async () => {
    ...
    await vi.waitFor(() => expect(sw.postMessage).toHaveBeenCalledWith(SW_TOKEN_EXPIRED_MSG));
  ...
  it('returns the original 401 when no real token change arrives before the timeout', async () => {
    ...
    const responsePromise = ev.respondWith.mock.calls[0][0] as Promise<Response>;
  ...
  it('passes non-401 responses through untouched (no client notification)', async () => {
    const sw = createFakeSelf();
    const fetchMock = vi.fn().mockResolvedValue(new Response('partial', { status: 206 }));
    vi.stubGlobal('fetch', fetchMock);
