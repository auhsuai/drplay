...
describe('truncatePathMiddle', () => {
  it('returns short paths unchanged without ellipsis', () => {
    ...
    expect(truncatePathMiddle('C:\\Users\\thinkpad\\Music')).toBe('C:\\Users\\thinkpad\\Music');
  ...
  it('keeps the final segment intact when truncating', () => {
    const long = 'C:\\Users\\thinkpad\\Desktop\\Antigravity\\drplay\\MyMusicFolder';
  ...
  it('handles mixed separators (both \\ and /)', () => {
    ...
    expect(truncatePathMiddle(mixed)).toBe('D:\\data\\…\\MyVeryLongDestinationFolder');
  });

  it('uses the actual separator of the path', () => {
    const posixFirst = 'D:/data/music/albums/MyVeryLongDestinationFolder';
    expect(truncatePathMiddle(posixFirst)).toBe('D:/data/…/MyVeryLongDestinationFolder');
  });

  it('handles UNC paths', () => {
    const unc = '\\\\server\\share\\Music\\albums\\MyMusicFolders';
    expect(truncatePathMiddle(unc)).toBe('\\\\server\\share\\…\\MyMusicFolders');
  });
