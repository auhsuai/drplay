...
//
// Extracted verbatim from driveRangeTokenizer.ts, which re-exports this API.
// The log source stays "driveRangeTokenizer" so existing error-log entries
// and tests keep matching. Cycle guard: this module must never import the
// tokenizer itself — it depends only on ./errorLog.
import { captureError } from "./errorLog";

// Log-source label kept identical to the tokenizer's own so error-log
// consumers (and tests asserting source) are unaffected by the extraction.
