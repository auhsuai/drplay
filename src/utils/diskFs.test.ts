...
describe("walkDiskFolder", () => {
  ...
  it("flattens the folder tree recursively, sorted by relativePath, sizes 0", async () => {
    ...
    expect(entries).toEqual([
      ...
      {
        path: "C:\\Music\\sub\\deep\\song3.ogg",
        name: "song3.ogg",
        relativePath: "sub/deep/song3.ogg",
        isDirectory: false,
        size: 0,
      },
      {
        path: "C:\\Music\\sub\\song2.flac",
        name: "song2.flac",
        relativePath: "sub/song2.flac",
        isDirectory: false,
        size: 0,
      },
      {
        path: "C:\\Music\\zebra.flac",
        name: "zebra.flac",
        relativePath: "zebra.flac",
        isDirectory: false,
        size: 0,
      },
    ]);
  ...
  it("throws AbortError immediately for an already-aborted signal (zero read_dir calls)", async () => {
    ...
    await expect(
      walkDiskFolder("C:\\Music", controller.signal),
  ...
  it("aborts mid-walk: signal aborted while a nested read_dir is pending → throws AbortError, no further read_dir", async () => {
    ...
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "plugin:fs|read_dir") {
        const path = pathOf(args);
        if (path === "C:\\Music")
          return Promise.resolve([dirEntry("sub", true)]);
...
describe("openDiskReadStream", () => {
  ...
  it("read() requests len = chunkSize and strips the trailing 8-byte nread", async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith("plugin:fs|read", {
      rid: RID,
      len: 1024,
    });
  ...
  it("close() invokes the core resources close command with the rid", async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith("plugin:resources|close", {
      rid: RID,
    });
  });
