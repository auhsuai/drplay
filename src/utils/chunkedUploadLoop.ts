// One session's worth of chunk PUTs for the chunked resumable upload
// (refactor: extracted from uploadFileResumableChunked.ts — the per-chunk
// retry PUT, the adaptive chunk-level math, the validated read, and the
// 2xx/308/404/409 session loop). Behavior identical to the pre-split inline
// versions; the facade re-exports nextChunkLevel so consumer imports keep
