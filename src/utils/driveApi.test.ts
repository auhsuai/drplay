...
// errorLog provides the canonical sanitizeString — keep the real one, mock
// only captureError (same pattern as apiClient.test.ts).
...
// not at error.reason. Both the retry path and the 403-specific handling
// share isRateLimit403Response through this shape.
...
// Shared retryable-status check for Drive retry loops: 429/5xx by status
// alone, 403 only when the body reports a Drive rate-limit reason (read via
// clone, only while retries remain so the final attempt never consumes the
// body).
