    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      try {
        ...
        void captureError({
          level: "warn",
          source: TOKENIZER_MODULE,
          message: `range-fetch-network-failed (fileId=${this.fileId}, bytes=${String(chunkStart)}-${String(chunkEnd)}): ${errorMessage(err)}`,
        });
      ...
      if (response.status === 206) {
        ...
        try {
          ...
          void captureError({
            level: "warn",
            source: TOKENIZER_MODULE,
            message: `range-fetch-body-failed (fileId=${this.fileId}, bytes=${String(chunkStart)}-${String(chunkEnd)}): ${errorMessage(err)}`,
          });
          throw new RangeFetchNetworkError("network", errorMessage(err));
        }
