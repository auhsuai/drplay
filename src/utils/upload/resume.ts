...
import { appendEntries, liveEntries } from "./queueState";
import {
  claimedPredecessorRowIds,
  claimResumedPredecessor,
  dbRowOp,
  resumeEntryFromRow,
} from "./session";
...
// concurrent pump as new uploads, so both can coexist safely.
...
export async function resumeInterruptedUploads(
  ...
  try {
    ...
    const claimedPredecessorIds = claimedPredecessorRowIds();
    for (const row of rows) {
      ...
      if (live.some((e) => e.id === row.id)) continue;
      ...
      claimResumedPredecessor(entry.id, row.id);
      resumed.push(entry);
    }
